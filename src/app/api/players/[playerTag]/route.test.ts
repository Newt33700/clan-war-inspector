/** Test du Route Handler GET /api/players/{playerTag} (US 9). */
...
describe('GET /api/players/[playerTag]', () => {
  beforeEach(() => {
  ...
  it('relaye le profil du joueur depuis Supercell', async () => {
    const urls: string[] = [];
    mockServer.use(
      http.get(`${SUPERCELL_API_BASE_URL}/players/*`, ({ request }) => {
        urls.push(request.url);
        return HttpResponse.json({ tag: '#P1', name: 'Joueur 1' });
      }),
    );
    ...
    expect(await response.json()).toEqual({ tag: '#P1', name: 'Joueur 1' });
    expect(urls[0]).toBe(`${SUPERCELL_API_BASE_URL}/players/%23P1`);
  });
